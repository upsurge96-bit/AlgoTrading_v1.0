...
middlewares.py
-------------
Security and production middlewares for the auth service.
"""
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        response.headers["Content-Security-Policy"] = (
            ...
            "img-src 'self' data:; "
            "connect-src 'self' https://kite.zerodha.com; "
...
def add_middlewares(app: FastAPI, origins: list = None) -> None:
    """
    Add all production middlewares to the app.
    
    Args:
        app: FastAPI application
        origins: Allowed CORS origins
    """
    # Default origins - restrict in production
    if origins is None:
        origins = ["http://localhost", "http://localhost:8018"]
    ...
    app.add_middleware(
        ...
        admin_rate_limit=300,  # 300 requests per minute for admin endpoints
        exclude_paths=["/health", "/metrics"]  # Don't rate limit these
    )
